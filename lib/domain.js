...
_.extend(Domain.prototype, {
    ...
    /**forked to add new domain path to explore tree structure**/
    addNewDomainPath: function (domainInstance, callback) {
        var self = domainInstance;
        var warnings = null;
        async.series([
            // load domain files...
            function (callback) {
                ...
                structureLoader(self.options.domainPath, self.validatorExtension, self.options.useLoaderExtensions, function (err, tree, warns) {
                    ...
                    tree._general.aggregates.forEach((newAggregates) => {
                        self.rawTree._general.aggregates.push(newAggregates);
                });
                    self.tree = attachLookupFunctions(self.rawTree);
                    callback(null);
                });
            },
            function (callback) {
    ...
    onDispatched: function (cmd, err, eventsToDispatch, aggregateData, meta, callback) {
        ...
        if (err) {
            ...
            if (cmdRejEvt) {
                this.onEventHandle(cmdRejEvt, function (err) {
                    if (err) {
                        debug(err);
                    }
                });
